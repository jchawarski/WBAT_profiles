...
wbat <- read.csv2("WBAT\\All_Sv_70_200kHz_newest\\St.61_70kHz_Sv_T20170904_14283000-20170904_14345302.txt", sep = ",", skip=6,header=FALSE, fill = TRUE)
...
## STOP ## How many digits does the "Time" column have? Adjust Accordingly
...
t <-1.94 *10^-5         # PulseCompressedEffectivePulseLength (sec)
...
d_scale <- seq(0, max(wbat.ctd$pressure), 25) # set depth scale for plotting
...
# comparison plot -- this plot shows both the original and the adjusted mean Sv value for the profile
 ggplot(wbat.ctd, aes(x=pressure, y=Estimate_new)) + 
  ...
  theme_bw()
       ...
       wbat.ctd$ctd_site <- "26DA.2017.9.288_110618"
       write.csv(wbat.ctd, "NEG2017_Stn.61_Gr.288_70kHz_WBAT-CTD-SV.csv")
...
ggplot(probe.all, aes(x=as.numeric(pressure), y=as.numeric(Estimate_new))) + geom_line(aes(colour=Frequency)) +
  #stat_smooth(formula = y ~ s(x, k = 45), method = "gam", se = T, linetype="dashed", color="black", aes(colour=Frequency)) + 
  coord_flip()+ scale_x_reverse() + xlim(250, 0) + labs(x= "Pressure [dBar]", y=Sv_label) + facet_wrap(~wbat_site) +
  theme_bw()
...
wbat.ts <- read.csv2("WBAT\\200kHz TS\\St.61_200kHz_TS_T20170904_14180756-20170904_14274200.txt", sep = ",", skip=19,header=FALSE, fill = TRUE)
# Give new names to columns:
...
write.csv(wbat.ctd, "NEG2017_Stn61._Gr.288_200kHz_WBAT-CTD-TS.csv")
